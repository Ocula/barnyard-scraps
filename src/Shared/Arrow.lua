...
function Arrow:Update()
	...
	if self._pointFromSubjectLookVector then
		_to = CFrame.new(self.Subject.Position, self.Subject.CFrame.lookVector + self.Subject.Position)
			* CFrame.new(1.5, 0, 0)
	end

	if self.Direction then
		_to = CFrame.new(self.Subject.Position, self.Direction + self.Subject.Position)
	...
	if self.Subject and self.Object then
		...
		local arrowPos = origin
			+ ((self.Subject.Size.Magnitude + self.Distance + math.sin(self.Move * self.AnimationOffset)) * direction)
