...
local function calculateUpStep(_dt: number, dummyCam) 
	local axis = dummyCam.CFrame.RightVector
...
return function(PlayerModule)
	...
	function baseCamera:GetCameraLookVector() 
		if not self._baseLookVector then 
			self._baseLookVector = Vector3.new(0,1,0) 
		end 

		return self._baseLookVector 
	end 
	...
	function baseCamera:GetDesiredPitchYaw()
		...
		if self._pitchYaw ~= self._lastPitchYaw then 
			self._lastPitchYaw = self._pitchYaw 
		end 
	...
	function baseCamera:SetDesiredLookVector(lV)
		...
		if lV ~= self._desiredLookVector then  
			...
			self._lastDesiredLookVector = lV 
		end 
	...
	function cameraObject:Update(dt: number)
		if self.activeCameraController then
			...
			calculateUpStep(dt, baseCamera:GetDummyCam()) 
