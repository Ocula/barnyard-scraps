...
function Place.new()
    ...
    self.YSpring = Spring(self._yOffset, 20, .6) 
    self.TwistSpring = Spring(self.Twist, 20, .5) 
...
function Place:Snap(object, pos): CFrame 
    if not self._currentFloor then return end 
...
function Place:Place(itemId, cf, objectId, config) 
    ...
    if GetItem then 
        ...
        self.Highlight:Flash(Color3.new(0,1,1), 1, .25, 0.1) 
...
function Place:SetInput() 
    ...
    self._InactiveMaid:GiveTask(Mouse.LeftDown:Connect(function(processed, guis)
        ...
        if Peek(self._input).Preferred == "Touch" then return end 
...
function Place:Update(object, forceLoad) 
    if not self._active and not forceLoad then return end 
    ...
    if object ~= nil and object:isDescendantOf(workspace) then
        ...
        if input.Preferred ~= "Touch" and input.Mouse == true then 
            ...
            if mousePos then 
                ...
                local twistCF = CFrame.Angles(math.rad(45),0,math.rad(45)):lerp(CFrame.Angles(0,0,0), Peek(self.TwistSpring))
                ...
                if snap then 
                    ...
                    local lerp = object.PrimaryPart.CFrame:lerp(snap * self._offset * CFrame.new(0,Peek(self.YSpring),0), 0.2)  
