...
function Mouse:GetPosition()
	local insetY, insetX = guiService:GetGuiInset() 
	return userInput:GetMouseLocation() - insetY 
end
...
function Mouse:GetRay(distance)
	local mousePos = self:GetPosition()
...
function Mouse:Raycast(raycastParams, distance, radius) 
	local mousePos = userInput:GetMouseLocation()
	local viewportMouseRay = cam:ViewportPointToRay(mousePos.X, mousePos.Y)
	return workspace:Spherecast(viewportMouseRay.Origin, radius or 0.1, viewportMouseRay.Direction * (distance or RAY_DISTANCE), raycastParams)
end
...
function Mouse:Start()
	...
	userInput.InputBegan:Connect(function(input, processed)
		...
		if (input.UserInputType == Enum.UserInputType.MouseButton1) then
			...
			local _getGuiObjects = playerGui:GetGuiObjectsAtPosition(currentPosition.X, currentPosition.Y) 
			
			self.LeftDown:Fire(processed, _getGuiObjects) 
	...
	userInput.InputChanged:Connect(function(input, processed)
		if (input.UserInputType == Enum.UserInputType.MouseMovement) then
			self.Moved:Fire(input, processed) 
