...
-- TODO: BETTER PORT OPTIONS.
-- 		We want to listen for connected gamepads and go from there. 
--		Future problem. 
...
local NumberUtil = require(Shared:WaitForChild("NumberUtil"))
...
function Gamepad:ApplyDeadzone(value, deadzoneThreshold)
	if (abs(value) < deadzoneThreshold) then
		...
		return NumberUtil.InverseLerp(deadzoneThreshold, 1, abs(value)) * (value > 0 and 1 or -1)
	end
end
