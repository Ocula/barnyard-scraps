...
return function(PlayerModule)
	...
	function cameraObject:SetFirstPerson(enabled: bool)
		if self.activeMouseLockController then 
			baseCamera:SetIsMouseLocked(enabled) 
	...
	function cameraObject:Reset(_setPitchYaw: Vector2) 
		targetUpVector = Vector3.new(0,1,0)

		if _setPitchYaw then 
			basePitchYaw = _setPitchYaw 
		end 
	...
	function cameraObject:Update(dt: number)
		if self.activeCameraController then
			...
			local lockOffset = self:IsMouseLocked() 
