...
function SummedCamera:__index(index)
	...
	elseif index == "Render" then
		if self._cameraA.Render then
			return self._cameraA.Render
		elseif self._cameraB.Render then
			return self._cameraB.Render
		end
	elseif SummedCamera[index] then
		return SummedCamera[index]
	else
		warn(("[SummedCamera] - '%s' is not a valid member"):format(tostring(index)))
	end
end
